...
static const uint8_t PANASONIC_AC_MIN_TEMPERATURE = 16; // Minimum temperature as reported by Panasonic app
static const uint8_t PANASONIC_AC_MAX_TEMPERATURE = 30; // Maximum temperature as supported by Panasonic app
static const float PANASONIC_AC_TEMPERATURE_STEP = 0.5; // Steps the temperature can be set in
static const float PANASONIC_AC_TEMPERATURE_TOLERANCE = 2; // The tolerance to allow when checking the climate state
...
class PanasonicACComponent : public Component, public UARTDevice, public Climate
{
  public:
    PanasonicACComponent(UARTComponent *parent) : UARTDevice(parent) {}

    void set_outside_temperature_sensor(Sensor *outside_temperature_sensor)
    {
      this->outside_temperature_sensor = outside_temperature_sensor;
    }

    void set_vertical_swing_sensor(TextSensor *vertical_swing_sensor)
    {
    ...
    void set_horizontal_swing_sensor(TextSensor *horizontal_swing_sensor)
    {
    ...
    void set_nanoex_switch(Switch *nanoex_switch)
    {
    ...
    ClimateTraits traits()
    {
      ...
      traits.set_visual_min_temperature(PANASONIC_AC_MIN_TEMPERATURE);
      traits.set_visual_max_temperature(PANASONIC_AC_MAX_TEMPERATURE);
      traits.set_visual_temperature_step(PANASONIC_AC_TEMPERATURE_STEP);
  ...
  private:
    Sensor *outside_temperature_sensor = NULL; // Sensor to store outside temperature from queries
    TextSensor *vertical_swing_sensor = NULL; // Text sensor to store manual position of vertical swing
    TextSensor *horizontal_swing_sensor = NULL; // Text sensor to store manual position of horizontal swing
    Switch *nanoex_switch = NULL; // Switch to toggle nanoeX on/off
    ...
    byte receiveBuffer[BUFFER_SIZE]; // Stores the packet currently being received
    ...
    void handle_init_packets()
    {
      ...
      {
        if(millis() - initTime > INIT_TIMEOUT) // Handle handshake initialization
        {
      ...
      else if(state == first_poll && millis() - lastPacketSent > FIRST_POLL_TIMEOUT) // Handle sending first poll
      {
      ...
      else if(state == handshake_ending && millis() - lastPacketSent > INIT_END_TIMEOUT) // Handle last handshake message
      {
    ...
    void handle_poll()
    {
      if(state == ready && millis() - lastPacketSent > POLL_INTERVAL)
      {
    ...
    bool verify_packet()
    {
      ...
      if(receiveBuffer[0] == 0x66) // Sync packets are the only packet not starting with 0x5A
      {
        ...
        initTime -= INIT_TIMEOUT; // Set init time back to trigger a initialization now
        receiveBufferIndex = 0; // Reset buffer
        return false;
      }

      if(receiveBuffer[0] != HEADER) // Check if header matches
      {
    ...
    void determine_action()
    {
      ...
      else if((this->mode == CLIMATE_MODE_COOL || this->mode == CLIMATE_MODE_AUTO) && this->current_temperature + PANASONIC_AC_TEMPERATURE_TOLERANCE >= this->target_temperature)
      {
        this->action = CLIMATE_ACTION_COOLING;
      }
      else if((this->mode == CLIMATE_MODE_HEAT || this->mode == CLIMATE_MODE_AUTO) && this->current_temperature - PANASONIC_AC_TEMPERATURE_TOLERANCE <= this->target_temperature)
      {
    ...
    void determine_fan_speed(byte speed)
    {
      switch(speed)
      {
        ...
        case 0x35: // 4
        case 0x36: // 5
          this->fan_mode = CLIMATE_FAN_HIGH; // Take 4 & 5 together for high since ESPHome doesn't have that many
        break;
        case 0x41: // Auto
    ...
    void handle_packet()
    {
      ...
      else if(receiveBuffer[2] == 0x10 && receiveBuffer[3] == 0x89) // Received query response
      {
        ...
        if(receiveBuffer[14] == 0x31) // Check if power state is off
          this->mode = CLIMATE_MODE_OFF; // Climate is off
        else
        {
          determine_mode(receiveBuffer[18]); // Check mode if power state is not off
        }

        this->target_temperature = receiveBuffer[22] * PANASONIC_AC_TEMPERATURE_STEP; // Set temperature
        ...
        for(int i = 0; i < receiveBuffer[10]; i++)
        {
          ...
          switch(receiveBuffer[currentIndex])
          {
            ...
            case 0x31: // Target temperature
              ...
              this->target_temperature = receiveBuffer[currentIndex + 2] * PANASONIC_AC_TEMPERATURE_STEP;
            break;
            case 0xA0: // Fan speed
    ...
    void handle_resend()
    {
      if(waitingForResponse && millis() - lastPacketSent > RESPONSE_TIMEOUT && receiveBufferIndex == 0) // Check if AC failed to respond in time and resend packet, if nothing was received yet
      {
    ...
    */
    ...
    {
      byte checksum = HEADER; // Checksum is calculated by adding all bytes together
