...
static const uint8_t HEADER =
    0x5A; // The header of the protocol, every packet starts with this

static const int INIT_TIMEOUT =
    10000; // Time to wait before initializing after boot
static const int INIT_END_TIMEOUT =
    10000; // Time to wait for last handshake packet
static const int FIRST_POLL_TIMEOUT =
    650; // Time to wait before requesting the first poll
static const int POLL_INTERVAL = 30000; // The interval at which to poll the AC
static const int RESPONSE_TIMEOUT =
    600; // The timeout after which we expect a response to our last command
static const int INIT_FAIL_TIMEOUT =
    30000; // The timeout after which the initialization is considered failed
...
enum class ACState {
  ...
  HandshakeEnding, // After the first poll, waiting for the last handshake
                   // packet
...
protected:
  ACState state_ = ACState::Initializing; // Stores the internal state of the
                                          // AC, used during initialization

  uint8_t transmit_packet_count_ =
      0; // Counter used in packet (2nd byte) when we are sending packets
  uint8_t receive_packet_count_ =
      0; // Counter used in packet (2nd byte) when AC is sending us packets

  const uint8_t
      *last_command_; // Stores a pointer to the last command we executed
  size_t
      last_command_length_; // Stores the length of the last command we executed

  uint8_t set_queue_[16]
                    [2]; // Queue to store the key/value for the set commands
  ...
  void send_command(const uint8_t *command, size_t commandLength,
                    CommandType type = CommandType::Normal);
