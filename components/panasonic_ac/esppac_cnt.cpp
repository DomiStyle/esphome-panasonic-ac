...
void PanasonicACCNT::control(const climate::ClimateCall &call) {
  ...
  if (call.get_custom_preset() != nullptr) {
    ...
    if (preset.compare("Normal") == 0)
      ...
      this->cmd[5] =
          (this->cmd[5] & 0xF0) + 0x02; // Clear right nib and set powerful mode
    else if (preset.compare("Quiet") == 0)
      this->cmd[5] =
          (this->cmd[5] & 0xF0) + 0x04; // Clear right nib and set quiet mode
    else
  ...
  if (set) {
    ...
    if (this->current_power_consumption_sensor_ != nullptr) {
      uint16_t power_consumption = determine_power_consumption(
          (int8_t)this->rx_buffer_[28], (int8_t)this->rx_buffer_[29],
          (int8_t)this->rx_buffer_[30]);
 ...
 */
void PanasonicACCNT::send_command(std::vector<uint8_t> command,
                                  CommandType type,
                                  uint8_t header = CNT::CTRL_HEADER) {
 ...
 */
void PanasonicACCNT::send_packet(const std::vector<uint8_t> &packet,
                                 CommandType type) {
...
bool PanasonicACCNT::verify_packet() {
  ...
  if (this->rx_buffer_[0] != CTRL_HEADER &&
      this->rx_buffer_[0] != POLL_HEADER) {
...
void PanasonicACCNT::handle_packet() {
  if (this->rx_buffer_[0] == POLL_HEADER) {
    this->data = std::vector<uint8_t>(this->rx_buffer_.begin() + 2,
                                      this->rx_buffer_.begin() + 12);
...
uint16_t PanasonicACCNT::determine_power_consumption(uint8_t byte_28,
                                                     uint8_t byte_29,
                                                     uint8_t offset) {
