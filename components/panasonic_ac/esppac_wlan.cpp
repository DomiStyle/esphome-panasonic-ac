...
void PanasonicACWLAN::loop() {
  if (this->state_ != ACState::Ready) {
    handle_init_packets(); // Handle initialization packets separate from normal
                           // packets
  ...
  if (millis() - this->last_read_ > READ_TIMEOUT &&
      !this->rx_buffer_
           .empty()) // Check if our read timed out and we received something
  {
    ...
    this->waiting_for_response_ =
        false; // Set that we are not waiting for a response anymore since we
               // received a valid one
    this->last_packet_received_ =
        millis(); // Set the time at which we received our last packet
...
void PanasonicACWLAN::control(const climate::ClimateCall &call) {
  ...
  if (call.get_mode().has_value()) {
    ...
    this->mode =
        *call.get_mode();  // Set mode manually since we won't receive a report
                           // from the AC if its the same mode again
    this->publish_state(); // Send this state, will get updated once next poll
                           // is executed
  }
  ...
  if (this->set_queue_index_ >
      0) // Only send packet if any changes need to be made
  {
    send_set_command();
  }
}
...
void PanasonicACWLAN::handle_poll() {
  if (this->state_ == ACState::Ready &&
      millis() - this->last_packet_sent_ > POLL_INTERVAL) {
...
void PanasonicACWLAN::handle_init_packets() {
  if (this->state_ == ACState::Initializing) {
    if (millis() - this->init_time_ >
        INIT_TIMEOUT) // Handle handshake initialization
    {
      ESP_LOGD(TAG, "Starting handshake [1/16]");
      send_command(CMD_HANDSHAKE_1,
                   sizeof(CMD_HANDSHAKE_1)); // Send first handshake packet, AC
                                             // won't send a response
      delay(3); // Add small delay to mimic real wifi adapter
      send_command(
          CMD_HANDSHAKE_2,
          sizeof(
              CMD_HANDSHAKE_2)); // Send second handshake packet, AC won't send
                                 // a response but we will trigger a resend

      this->state_ = ACState::Handshake; // Update state to handshake started
    }
  } else if (this->state_ == ACState::FirstPoll &&
             millis() - this->last_packet_sent_ >
                 FIRST_POLL_TIMEOUT) // Handle sending first poll
  {
    ...
    this->state_ = ACState::HandshakeEnding;
             ...
             millis() - this->last_packet_sent_ >
                 INIT_END_TIMEOUT) // Handle last handshake message
  {
...
bool PanasonicACWLAN::verify_packet() {
  ...
  if (this->rx_buffer_[0] ==
      0x66) // Sync packets are the only packet not starting with 0x5A
  {
    ESP_LOGI(TAG, "Received sync packet, triggering initialization");
    this->init_time_ -=
        INIT_TIMEOUT; // Set init time back to trigger a initialization now
    this->rx_buffer_.clear(); // Reset buffer
    return false;
  }
  ...
  if (this->state_ == ACState::Ready &&
      this->waiting_for_response_) // If we were waiting for a response, check
                                   // if the tx packet counter matches (if we
                                   // are ready)
  {
        ...
        this->rx_buffer_[1] != 0xFE) // Check transmit packet counter
    {
      ...
      this->receive_packet_count_ = this->rx_buffer_[1];
             ...
             ACState::Ready) // If we were not waiting for a response, check if
                             // the rx packet counter matches (if we are ready)
  {
    if (this->rx_buffer_[1] !=
        this->receive_packet_count_) // Check receive packet counter
    {
...
void PanasonicACWLAN::handle_packet() {
  ...
  } else if (this->rx_buffer_[2] == 0x10 &&
             this->rx_buffer_[3] == 0x89) // Received query response
  {
    ...
    if (this->rx_buffer_[14] == 0x31)         // Check if power state is off
      ...
      this->mode = determine_mode(
          this->rx_buffer_[18]); // Check mode if power state is not off
    }
    ...
    update_outside_temperature(
        (int8_t)this->rx_buffer_[66]); // Set current (outside) temperature

    std::string horizontalSwing =
        determine_swing_horizontal(this->rx_buffer_[34]);
    ...
    // climate::ClimateAction action = determine_action(); // Determine the
    // current action of the AC this->action = action;
             ...
             this->rx_buffer_[3] == 0x0A) // Report
  {
    ...
    for (int i = 0; i < this->rx_buffer_[10]; i++) {
      ...
      case 0xA0: // Fan speed
        ...
        this->custom_fan_mode =
            determine_fan_speed(this->rx_buffer_[currentIndex + 2]);
        break;
      case 0xB2: // Preset
        ESP_LOGV(TAG, "Received preset");
        this->custom_preset =
            determine_preset(this->rx_buffer_[currentIndex + 2]);
        break;
      case 0xA1:
      ...
      case 0xA5: // Horizontal swing position
        ...
        update_swing_horizontal(
            determine_swing_horizontal(this->rx_buffer_[currentIndex + 2]));
        break;
      case 0xA4: // Vertical swing position
        ESP_LOGV(TAG, "Received vertical swing position");

        update_swing_vertical(
            determine_swing_vertical(this->rx_buffer_[currentIndex + 2]));
        break;
    ...
    this->publish_state();
             ...
             this->rx_buffer_[3] == 0x80) // Answer for handshake 16
  {
...
void PanasonicACWLAN::handle_handshake_packet() {
                 ...
                 0x09) // First unsolicited packet from AC containing rx counter
  {
    ...
    send_command(CMD_HANDSHAKE_14, sizeof(CMD_HANDSHAKE_14),
             ...
             this->rx_buffer_[3] == 0x20) // Second unsolicited packet from AC
  {
...
void PanasonicACWLAN::send_set_command() {
  // Size of packet is 3 * 4 (for the header, packet size, and key value pair
  // counter) setQueueIndex * 4 for the individual key value pairs
  ...
  packet[5] =
      (4 * this->set_queue_index_) - 1 +
      6; // Packet size, key value pairs * 4, subtract checksum (-1), add 4
  ...
  for (int i = 0; i < this->set_queue_index_; i++) {
    ...
    packet[12 + (i * 4) + 3] =
        0x00; // Unknown, either 0x00 or 0x01 or 0x02; overwritten by checksum
              // on last key value pair
  }
...
void PanasonicACWLAN::send_command(const uint8_t *command, size_t commandLength,
                                   CommandType type) {
  std::vector<uint8_t> packet(commandLength +
                              3); // Reserve space for upcoming packet
  ...
  this->last_command_length_ =
      commandLength; // Store the length of the last command we sent

  send_packet(packet, type); // Actually send the constructed packet
}

void PanasonicACWLAN::send_packet(std::vector<uint8_t> packet,
                                  CommandType type) {
  ...
  if (type == CommandType::Response)
    packetCount =
        this->receive_packet_count_; // Set the packet counter to the rx counter
  else if (type == CommandType::Resend)
    packetCount = this->transmit_packet_count_ -
                  1; // Set the packet counter to the tx counter -1 (we are
  ...
  this->last_packet_sent_ =
      millis(); // Save the time when we sent the last packet

  if (type == CommandType::Normal) // Do not increase tx counter if this was a
                                   // response or if this was a resent packet
  {
    if (this->transmit_packet_count_ == 0xFE)
      this->transmit_packet_count_ =
          0x01; // Special case, roll over transmit counter after 0xFE
    else
      this->transmit_packet_count_++; // Increase tx packet counter if this
                                      // wasn't a response
  } else if (type == CommandType::Response) {
    if (this->receive_packet_count_ == 0xFE)
      this->receive_packet_count_ =
          0x01; // Special case, roll over receive counter after 0xFE
    else
      this->receive_packet_count_++; // Increase rx counter if this was a
                                     // response
  }

  if (type != CommandType::Response) // Don't wait for a response for responses
    this->waiting_for_response_ =
        true; // Mark that we are waiting for a response
...
void PanasonicACWLAN::handle_resend() {
  if (this->waiting_for_response_ &&
      ...
      this->rx_buffer_.empty()) // Check if AC failed to respond in time and
                                // resend packet, if nothing was received yet
  {
    ESP_LOGD(TAG, "Resending previous packet");
    send_command(this->last_command_, this->last_command_length_,
                 CommandType::Resend);
  }
}
